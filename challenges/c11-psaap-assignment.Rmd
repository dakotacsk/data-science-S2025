...
-   There are 22 columns in df_psaap, and 140 rows. All of the columns are numeric and stored in doubles. The dataset does not contain any missing values.
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   `fit_cheat` is the most accurate; `fit_nonphysical` is the least accurate.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   `avg_T` is the Plane-averaged fluid temperature, which is an *Output* variable. It is effective because it is directly related to the output `T_norm`.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No. We calculate avg_T based on the output `T_norm`, so we would not have access to it before we guessed what `T_norm` should be. It is not a valid predictor.
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   `idx` is the id of the row number. It is a metadata variable and does not have any physical meaning.
...
-   Which columns are excluded in the model formula above? What categories do these belong to? Why are these important quantities to leave out of the model?
    -   `idx`, `avg_q`, `avg_T`, and `rms_T` are excluded. `idx` is a metadata variable, while `avg_q`, `avg_T`, and `rms_T` are outputs. These are important to leave out of the model because metadata does not have a physical meaning and the outputs are not available before we make the prediction.
-   Which inputs are *statistically significant*, according to the model?
    -   According to the p-values (assuming p\<0.05 is significant), `x`, `L`, `W`, `U_0`, `C_fp`, `d_p` and `I_0` are all statistically significant.
-   What is the regression coefficient for `x`? What about the regression coefficient for `T_f`?
    -   The regression coefficient for `x` is 1.018323e+00. The regression coefficient for `T_f` is -3.791436e-04 .
-   What is the standard deviation of `x` in `df_psaap`? What about the standard deviation of `T_f`?
    -   The standard deviation of `x` is 0.2805121 and the standard deviation of `T_f` is 38.94204.
-   How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
    -   `x` has a large coefficient (1.02), but it has a small standard deviation (0.28). `T_f` has a tiny coefficient (-0.00038) and a large SD (38.94). This means that while `x` has a seemingly larger impact, due to the small std, it only causes a change of \~0.286 units per one std unit change. `T_f` has a small impact, and even with the large std it barely has a real world effect.
-   Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
    -   Just because something is statistically significant doesn't mean it has a meaningful or practical effect. With enough data, even tiny, irrelevant effects can show up as "significant." It’s more useful to consider effect size in context—like how a coefficient actually translates to changes in the output given the input’s variability. This is also a good time to think about causation vs correlation. Just because two variables are correlated doesn't mean one causes the other.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   q4 is more accurate. If we look at the graph, we can see that model q4's predictions are much closer to the actual T_norm compared to model x's predictions. We also see that there seems to only four main values that model x predicts, with it capping at 1 while T_norm actual increases.
...
-   How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
    -   `fit_simple` only uses 1 prediction, `x`, whereas `fit_q4` uses every predictor but `idx`, `avg_q`, `avg_T`, and `rms_T`. This means fit_q4 uses 17 predictors.
...
-   How much do you trust your model? Why?
    -   I trust it moderately. It is a simple model that only uses 4 predictors, and it is based on the training data. However, I am not sure how well it will generalize to new data.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   I would use the prediction interval because the team is trying to predict the range of actual T_norm outcomes in the real world, not just the mean trend. By using the prediction interval we can account for real-world randomness.
-   What fraction of validation cases lie within the intervals you predict? (NB. Make sure to calculate your intervals *based on the validation data*; don't just use one single interval!) How does this compare with `pr_level`?
    -   About 93.3% of validation cases fall within the predicted 80% prediction interval. This means the interval is conservative (a bit wider than necessary) but that's not a bad thing for engineering design where safety margins are valuable.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   I would recommend they plan around a T_norm range of [1.457, 2.296], which gives them a reliable performance band based on current design settings and typical variability in the other factors.
-   Are there any other recommendations you would provide?
    -   Yes. I would recommend that the design team consider the impact of the other impactful variables (e.g.  I_0, d_p, or h). Something like a sensitivity analysis could help figure out which one is the most important one to focus on.
