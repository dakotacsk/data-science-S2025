...
- You chose a correct value of `stat(x, y)` when `x, y` is *outside* the circle. Why did you choose this value?
  - When it lands outside of the circle, it is essentially a "miss" in our Monte Carlo simulation. We want to count the number of points that land inside the circle, so we return 0 for points outside the circle. This way, when we average these values, they do not contribute to our estimate of pi.
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the circle. Why did you choose this value?
  - The area of a quarter circle (denoted S_c) is pi/4, so with Monte Carlo estimation, we can deduce that pi should be equal to 4 \* (fraction of points that land in S_c). This means, we should return 4 when x^2 + y^2 <= 1 so that it contributes 4 to the sum and when we average these we get 4 \* (fraction of points in S_c) which is our pi estimation.
...
- What is a range of plausible values, based on the sampling distribution you've generated?
  - The range of plausible values for π based on this sampling distribution appears to be approximately between 3.048	- 3.264	.
...
- How closely do your bootstrap CI and CLT CI agree?
  - Bootstrap range is 3.048 - 3.264, CLT range is 3.054794-3.257206. They both are very close to each other, with the bootstrap CI being slightly wider.
...
- Comment on the width of your CI(s). Would your estimate of $\pi$ be good enough for roughly estimating an area (e.g., to buy enough paint for an art project)? Would your estimate of $\pi$ be good enough for precisely calculating a trajectory (e.g., sending a rocket into orbit)?
  - It is good enough for rough estimations. Depending on use case, we might want to overshoot or undershoot based on the range.
  - For precise calculations, we would need a much narrower CI. The current CI is too wide to be useful for precise calculations, such as sending a rocket into orbit. The rocket would probably fly out of orbit if we used the current CI.
  
- What would be a *valid* way to make your CI more narrow?
  - More data points would help narrow the CI. We could also use a more precise method of estimating pi, such as using a higher number of points in the Monte Carlo simulation or using a different method altogether.
