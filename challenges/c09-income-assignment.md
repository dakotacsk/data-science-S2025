...
filename_pop <- 'ACSDT5Y2018.B01003-Data.csv' # replace with your filename
...
df_q4 <- df_q3 %>%
  mutate(
    income_SE = income_moe / 1.645,  # Convert MOE to SE
    income_lo = income_estimate - 2.576 * income_SE,  # 99% CI lower bound
    income_hi = income_estimate + 2.576 * income_SE,  # 99% CI upper bound
    income_CV = income_SE / income_estimate  # Coefficient of Variation
  )
```
...
- Document your observations here.
  ...
  - It seems that most 2-person families have a very low margin of error
    compared to the 5 or 6-person families, indicating that the
    estimates for 2-person families are more precise. We can also see
    that 2-person-families tend to make less than families with more
    people. There is also a very large margin for Nantucket across all
    categories compared to other counties, indicating that the estimates
    for Nantucket are less precise.

- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?

  - No, we cannot confidently distinguish between household incomes in
    Suffolk county because the confidence intervals for the different
    family sizes overlap significantly. For example, the true median
    income for 2-person families could be less than, equal to, or
    greater than that of 3-person families, and vice versa. The
    overlapping confidence intervals indicate that the estimates are not
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - There is an overall trend of decreasing standard error (SE) as
...
- What does this *overall* trend tell you about the relative ease of
  ...
  - It suggests that studying small counties is generally more difficult
    in terms of obtaining precise estimates, as they tend to have larger
    standard errors. This is likely due to the smaller sample sizes and
    greater variability in the data for small populations. In contrast,
    larger counties provide more data points, leading to more reliable
    and precise estimates. Basically, smaller counties are harder to
    study and get precise results.
...
``` r
# Are counties with lower than average income more likely to have smaller families?

# Define income thresholds (top/bottom 20%)
income_cutoffs <- df_data %>%
  summarise(
    bottom_20 = quantile(income_estimate, 0.2, na.rm = TRUE),
    top_20 = quantile(income_estimate, 0.8, na.rm = TRUE)
  )
...
df_data %>%
  mutate(
    income_group = case_when(
      income_estimate <= income_cutoffs$bottom_20 ~ "Bottom 20%",
      income_estimate >= income_cutoffs$top_20 ~ "Top 20%",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(income_group)) %>%
  group_by(category, income_group) %>%
  summarise(
    count = n(),
    .groups = "drop"
  ) %>%
  ...
  mutate(
    ...
    category = fct_relevel(category, "1-person families", "2-person families", 
                          "3-person families", "4-person families")
  ) %>%
  ggplot(aes(x = category, y = pct, fill = income_group)) +
  ...
  scale_fill_manual(
    values = c("Top 20%" = "#bbddff", "Bottom 20%" = "#ffbbdd"),
    name = "Income Group:"
  ) +
  labs(
    x = "Family Size",
    y = "Percentage of Counties",
    title = "Family Size Distribution in Highest vs Lowest Income Counties",
    subtitle = "Counties in top and bottom 20% of median income"
  ) +
  theme_minimal() +
  theme(
    legend.position = "top",
...
```
    ...
    ## The first warning was:
    ## ℹ In argument: `category = fct_relevel(...)`.
    ## ℹ In group 1: `income_group = "Bottom 20%"`.
    ## Caused by warning:
    ## ! 1 unknown level in `f`: 1-person families
...
- Bigger households tend to reside in counties with higher median
  incomes, while smaller households are more common in counties with
  lower median incomes. This trend is consistent across the top and
  bottom 20% of income groups. This can be hypothesized as larger
  households cost more, and thus tend to be found in wealthier areas
  where families can afford larger living spaces and expenses.
  Conversely, smaller households may be more prevalent in lower-income
  areas due to economic constraints, as individuals or couples may not
  have the financial means to support larger families.
